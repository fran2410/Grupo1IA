...
INFO_A_MANO = {
  ...
  "2505.02271v1.xml": {
    ...
    "organizaciones": [
      ...
      "European Commission",
      "Chips Act Joint Undertaking project"
    ]
  },
  "2505.07105v1.xml": {
    "autores_articulo": [],
...
def process_xml_directory(xml_dir, perform_evaluation):
        ...
        try:
            ...
            output_data_for_json[filename] = {
                ...
                'predictions_by_model': {
                    model: {
                        'personas': sorted(list(preds.get('personas', set()))) if preds.get('personas') else [],
                        'organizaciones': sorted(list(preds.get('organizaciones', set()))) if preds.get('organizaciones') else []
    ...
    with open(os.path.join("entrenamiento", "Text_Extraction_results_detailed.json"), "w", encoding="utf-8") as f:
