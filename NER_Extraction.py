...
def split_text(text, max_tokens=256):
    """Divide el texto en chunks manejables manteniendo integridad de oraciones"""
...
def extract_text_from_grobid_xml(xml_path):
    """Extrae el texto completo de la sección de agradecimientos (acknowledgements) en XML generado por GROBID,
    incluyendo el texto dentro de los elementos <rs>.
    
    Args:
        xml_path (str): Ruta al archivo XML
        
    Returns:
        str: Texto completo de los agradecimientos o cadena vacía si no se encuentra
    """
    ...
    # Buscar la sección de acknowledgements
    acknowledgement_div = root.find('.//tei:back/tei:div[@type="acknowledgement"]', namespaces)
    
    if acknowledgement_div is None:
        return ""
    
    # Función recursiva para extraer todo el texto, incluyendo el de los elementos hijos
    ...
    # Extraer todo el texto de los párrafos de agradecimientos
